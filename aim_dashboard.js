...
function render(){
 ...
 <div class="card"><h3>Recent Allocation Activity</h3>${acts.length?`<table><tr><th>Date</th><th>Store</th><th>Product</th></tr>${acts.map(x=>`<tr><td>${esc(x.effective_date)}</td><td>${esc(x.store)}</td><td><a href="#" data-product-name="${esc(x.product)}">${esc(x.product)}</a></td></tr>`).join("")}</table>`:'<p class="empty">No recent store-level allocation evidence in AIM.</p>'}</div></div>
 <div class="card"><h3>Latest NC ABC Shipment Intelligence</h3>${ships.length?`<table><tr><th>Shipment date</th><th>Product</th><th>Cases</th></tr>${ships.map(x=>`<tr><td>${esc(x.date)}</td><td><a href="#" data-product-name="${esc(x.product)}">${esc(x.product)}</a></td><td>${esc(x.cases)}</td></tr>`).join("")}</table>`:'<p class="empty">No matched shipment intelligence currently available.</p>'}</div>`;
 if(window.AIMProducts) AIMProducts.ready.then(()=>AIMProducts.linkify(document.getElementById("view"))).catch(()=>{});
}
