...
function render(){
 ...
 <div><span class="status ${esc(b.activity_status)}">${esc(b.activity_label)}</span>${inv}</div>
 ...
 <div class="sub"><b>Official Board information</b><p>${esc(b.contact.phone)||"—"}<br>${esc(b.contact.email)||"—"}</p><span class="note">${esc(b.public_notes)}</span></div></div></div>
